...
optimization method that many times. Useful with methods such as Differential
Evolution, when restarting the method when the random seed has changed might
yield a different result. The user can retrieve the runs with the best overall
value from the member `$bestRuns` in the result. See the section "Restarting the
optimization multiple times" below for more information.
- `progressUpdate`: If greater than `0`, prints a progress update to the console
each time that many runs of the optimization procedure have completed.
...
Now we are ready to calculate the confidence regions. We choose the standard
value for the confidence level equal to $0.95$, but this can be modified.
...
generating the confidence intervals, it is also possible to use the cube-root
bootstrap method proposed by Cattaneo, Jansson and Nagasawa (2020). The
