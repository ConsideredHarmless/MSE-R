...
objective <- makeObjFun(dataArray, coefficient1)
# Set seed once before the rest.
randomSeed <- 42
set.seed(randomSeed)
optimResult <- maximizeNew(objective, optimParams)
print(optimResult)
```
...
ssSize <- 2
optionsCR <- list(progressUpdate=0, confidenceLevel=0.95, asymptotics="nests")
numSubsamples <- 50
pointEstimate <- as.numeric(optimResult$bestmem)
cr <- pointIdentifiedCRNew(ssSize, numSubsamples, pointEstimate, numFreeAttrs,
                           groupIDs, dataArray, optimParams, optionsCR)
plotCR(cr$estimates)
```
